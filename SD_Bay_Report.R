...
# fill in haplotypes with the most recent...
# uniq.turtles <- na.omit(unique(turtle.haplo.SDB$Turtle_ID))
#
# k <- 1
# for (k in 1:length(uniq.turtles)){
#   tmp <- filter(turtle.haplo.SDB,
#                 Turtle_ID == uniq.turtles[k])
#   if(nrow(tmp) > 1) stop()
#
#   na.haplo <- is.na(tmp$Haplo)
#   if (sum(na.haplo) > 0 & sum(na.haplo) < length(na.haplo)){
#     haplos <- tmp$Haplo[!is.na(tmp$Haplo)]
#     if (length(haplos) > 0){
#       best.haplo <- haplos[length(haplos)]
#
#     }
#   } else if (sum(na.haplo) == 0) {  # if no NA in haplo field
#     haplos <- tmp$Haplo
#     best.haplo <- haplos[length(haplos)]
#
#   }
#   turtle.haplo.SDB[turtle.haplo.SDB$Turtle_ID == uniq.turtles[k] &
#                      !is.na(turtle.haplo.SDB$Turtle_ID)&
#                      is.na(turtle.haplo.SDB$Haplo), 'Haplo'] <- best.haplo
#
# }
