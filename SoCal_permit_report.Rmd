...
# Seal Beach data - this seems to not work...
# if (!file.exists(paste0("~/R/Cm_SealBeach/data/SB_report_", Sys.Date(), ".csv"))){
#   source("~/R/Cm_SealBeach/SealBeach_Report.R")
# }
...
knitr::kable(closed.model.table, 
             digits = 1,
             caption = "Table 2. A comparison of two closed models that were fitted to capture-recapture histories of green turtles in San Diego Bay during the abundance-estimation study in 2019. p = capture/recapture probability, f0 = abundance")

```


```{r Huggins_model_table, echo=FALSE, message=FALSE}
knitr::kable(Huggins.model.table, 
             digits = 1,
             caption = "Table 3. A comparison of six Huggins' closed models that were fitted to capture-recapture histories of green turtles in San Diego Bay during the abundance-estimation study in 2019. p = capture/recapture probability. Cat1, Cat2, and Cat3 indicates binary categorical variable, where CCL was grouped by cutoff values of 70, 80, and 90 cm, respectively.")
...
Goodness of fit tests indicated that the closed model was appropriate for the data where no test indicated a significant departure from the expected numbers of recaptures (results not shown). No goodness of fit tests are available for Huggins' closed models. 
...
#### Somatic growth
Because our capture-recapture effort started in 1990, body size of captured turtles in the reporting period (from 1 October 2018 to 30 September 2019) was examined in the context of the entire dataset. Missing SCL (cm) or CCL (cm) was predicted using the regression equation in Eguchi et al. (2010: CCL = 0.64 + 1.06SCL, n = 211, r^2^ = 0.99). Missing body mass (kg) was also predicted using the regression equation in Eguchi et al. (2010: ln(Mass) = 0.96 + 0.04SCL, n = 177, r^2^ = 0.96).

Standard carapace length of captured turtles during the reporting period ranged from ```r min(unique.new.captures.SDB$SCL, na.rm = T)``` cm to ```r max(unique.new.captures.SDB$SCL, na.rm = T)``` cm (mean = ```r signif(mean(unique.new.captures.SDB$SCL), 3)``` cm, SE = ```r signif(SE(unique.new.captures.SDB$SCL), 2)``` cm, n = ```r nrow(unique.new.captures.SDB)```, whereas for the entire dataset mean = ```r signif(mean(turtle.haplo.SDB$SCL, na.rm = T), 3)``` cm and SE = ```r signif(SE(turtle.haplo.SDB$SCL), 3)``` cm (n = ```r nrow(turtle.haplo.SDB)```. 

```{r change_SCL, echo=FALSE, warning=FALSE, fig.cap= "Figure 1. Changes in SCL (cm) of green turtles captured in San Diego Bay from 1990 to present. The current reporting period is shown as the shaded region. Lines connect the recaptured individuals."}
estim.SCL <- filter(turtle.haplo.SDB, SCL.regress == 1)

ggplot()  +
  geom_path(data = turtle.haplo.SDB,
            aes(x = Date, y = CCL, group = Turtle_ID)) + 
...
To investigate the change in size (CCL) of green turtles in SDB, we compared CCL of first captures of all turtles over the study period.  Distributions of CCL appeared to change over time, where a greater proportion of smaller individuals (< 80 cm) was found in the last 5 years than before (Figure 2). This may be a result of the increasing population.

```{r change_size, echo = FALSE, warning=FALSE, message=FALSE, fig.cap="Figure 2. Changes in SCL (cm) distribution of green turtles captured in San Diego Bay in 5-year segments from 1990 to present. Data points for each 5-year segment are shown as a rug plot."}
...
During the reporting period, ```r length(unique(new.captures.SB$Date))``` field outings were made to the SBNWR between ```r min(new.captures.SB$Date)``` and ```r max(new.captures.SB$Date)``` and there were a total of ```r nrow(new.captures.SB)``` green turtle interactions. 

Of the ```r nrow(new.captures.SB)``` interactions in the SBNWR with ```r length(unique(new.captures.SB$Turtle_ID))``` unique animals, ```r nrow(filter(unique.new.captures.SB, Sex == "F"))``` of these were classified as females and the rest were classified as juveniles with an unknown sex (Table 1), based upon the mean nesting size eastern Pacific green turtles.  Of the ```r length(unique(new.captures.SB$Turtle_ID))``` turtles,  ```r nrow(new.turtles.SB)``` were new animals not previously captured (Table 5).  
...
#### Somatic growth
Because our capture-recapture effort started in ```r min(year(turtle.haplo.SB$Date))```, body size of captured turtles in the reporting period was examined in the context of the entire dataset. Missing SCL (cm) or CCL (cm) was predicted using the regression equation in Eguchi et al. (2010: CCL = 0.64 + 1.06SCL, n = 211, r^2^ = 0.99). Missing body mass (kg) was also predicted using the regression equation in Eguchi et al. (2010: ln(Mass) = 0.96 + 0.04SCL, n = 177, r^2^ = 0.96).

Standard carapace length of captured turtles during the reporting period ranged from ```r min(unique.new.captures.SB$SCL, na.rm = T)``` cm to ```r max(unique.new.captures.SB$SCL, na.rm = T)``` cm (mean = ```r signif(mean(unique.new.captures.SB$SCL), 3)``` cm, SE = ```r signif(SE(unique.new.captures.SB$SCL), 2)``` cm, n = ```r nrow(unique.new.captures.SB)```, whereas for the entire dataset mean = ```r signif(mean(turtle.haplo.SB$SCL, na.rm = T), 3)``` cm and SE = ```r signif(SE(turtle.haplo.SB$SCL), 3)``` cm (n = ```r nrow(turtle.haplo.SB)```). 

```{r Change_SCL_SB, echo=FALSE, warning=FALSE, echo=FALSE, warning=FALSE, fig.cap= "Figure 3. Changes in SCL (cm) of green turtles captured in Seal Beach National Wildlife Refuge from 2010 to present. The current reporting period is shown as the shaded region. Lines connect the recaptured individuals."}
...
# Literature Cited:
CCOOCH E, WHITE G. Program MARK. Available from http://www.phidot.org  
