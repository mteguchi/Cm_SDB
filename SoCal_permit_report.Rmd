...
col_def_SDB <- cols(NMFS_Tag = col_factor(),
                    PIT_LFF = col_factor(),
                    PIT_RFF = col_factor(),
                    Tag_LFF = col_factor(),
                    Tag_RFF = col_factor(),
...
turtle.haplo.SDB <- read_csv(file = paste0("data/SDB_report_", Sys.Date(), ".csv"),
...
col_def_SB <- cols(Date = col_date(format = "%Y-%m-%d"),
                    PIT_LFF = col_factor(),
                    PIT_RFF = col_factor(),
                    Tag_LFF = col_factor(),
                    Tag_RFF = col_factor(),
...
turtle.haplo.SDB %>% group_by(NMFS_Tag) %>%
  mutate(n = n()) -> turtle.haplo.SDB
...
new.captures.SDB %>% group_by(NMFS_Tag) %>%
...
Field operations were conducted under NMFS Scientific Research Permit 18238 in Seal Beach Wildlife Refuge between ```r min(new.captures.SB$Date)``` and ```r max(new.captures.SB$Date)```.  During the permit year, ```r length(unique(new.captures.SB$Date))``` field outings were made to Seal Beach Wildlife Refuge with a total of ```r nrow(new.captures.SB)``` green turtle interactions. 
...
Of the ```r nrow(new.captures.SB)``` interactions in San Diego Bay with ```r length(unique(new.captures.SB$Turtle_ID))``` unique animals, ```r nrow(filter(unique.new.captures.SB, Sex == "F"))``` of these were classified as females, ```r nrow(filter(unique.new.captures.SB, Sex == "M"))``` were classified by males, and ```r nrow(filter(new.captures.SB, Sex == "U"))``` were juveniles with an unknown sex (Table 1), based upon the mean nesting size eastern Pacific green turtles.  X were new animals not previously captured (See Table 1 with *).  This group of previously untagged turtles likely consists of recent recruits into the area, i.e. turtles that have not been living in the area for long and have recently made the life-history shift from being juveniles in the pelagic ocean to larger sub-adults in the near-shore environment, as well as larger animals that have not been captured previously. 

```{r echo=FALSE}
new.captures.SB %>% 
  select(Turtle_ID, Date, Sex, Weight, SCL) %>%
  arrange(by = Date) -> new.captures.SB.table
knitr::kable(new.captures.SB.table, format = "markdown", digits = 1,
...
```{r Change_SCL_SB, echo=FALSE, warning=FALSE, echo=FALSE, warning=FALSE, fig.cap= "Figure 2. Changes in SCL (cm) of green turtles captured in Seal Beach National Wildlife Refuge from YR to present. The current reporting period is shown as the shaded region. Lines connect the recaptured individuals."}
estim.SCL.SB <- filter(turtle.haplo.SB, SCL.regress == 1)

ggplot()  +
  geom_path(data = turtle.haplo.SB,
            aes(x = Date, y = SCL, group = Turtle_ID)) + 
